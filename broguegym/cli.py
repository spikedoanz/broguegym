"""Terminal frontend for playing Brogue through the Gymnasium environment."""
...
from broguegym.spaces import ObservationMode
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    observation_mode: ObservationMode = "privileged" if args.tensors else "player"
    env = BrogueEnv(
        render_mode="ansi",
    ...
    try:
        ...
        if events is not None:
            for event in events:
                step_count, last_observation = _step(
                    env,
                    action_lookup,
                    step_count,
                    event,
...
def _parse_args(argv: Sequence[str] | None) -> _CliArgs:
    parser = argparse.ArgumentParser(description="Play Brogue through the Gymnasium environment.")
    ...
    return _CliArgs(
        seed=cast(int | None, values["seed"]),
        actions=cast(str | None, values["actions"]),
        tensors=cast(bool, values["tensors"]),
    )
...
def _emit(
    ...
    *,
    ...
    payload = {
        ...
        "info": {str(k): v.value if isinstance(v, Enum) else v for k, v in info.items()},
    }
    ...
    if tensors:
        tensor_observation = {
            key: value for key, value in observation.items() if key.startswith(TENSOR_PREFIXES)
        }
        sys.stdout.write(repr(tensor_observation) + "\n")
...
def _scripted(actions: str | None) -> Iterator[KeyEvent] | None:
    if actions is None:
        return None
    return (_event(bytes((ord(char),))) for char in actions)
